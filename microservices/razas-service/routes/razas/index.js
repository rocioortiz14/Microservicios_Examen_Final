...
router.get("/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const raza = await razas.findById(id);
    if (!raza) {
      return res.status(404).send({ message: `No se encontró ningún registro con el id ${id}` });
    }
    const response = await fetch(`http://perros:3000/api/v2/perros/${id}`);
    const perros = await response.json();
    return res.status(200).send({ raza, perros });
  } catch (err) {
    console.error(`Error retrieving by id: ${err.message}`);
    return res.status(500).send({ message: `Error al buscar la raza con el id ${id}` });
  }
});
